...
// This device is read purely over HID for normal use — the CDC port is only ever
// opened for one-off config, so it can't tell us whether the flight-sim app is
// actually using the device. The best available signal is USB enumeration itself:
// ARDUINO_USB_STARTED_EVENT fires once the host finishes recognizing the composite
// device (tud_mount_cb), ARDUINO_USB_STOPPED_EVENT if it's unplugged or the bus
// drops (tud_umount_cb). So "connected" here means "enumerated by a host", not "the
// app has it open" — there's no such signal for a plain HID gamepad with no
